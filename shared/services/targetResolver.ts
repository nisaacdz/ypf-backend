...
export async function resolveAudience(
  ...
  if ("op" in rule) {
    return resolveCompositeRule(rule);
  } else {
    return resolveAtomicRule(rule);
  }
}
...
async function resolveAtomicRule(rule: AtomicRule): Promise<Set<string>> {
  switch (rule.kind) {
    case "CONSTITUENTS":
...
async function resolveAllConstituents(
  ...
  // Note: 'isActive' is not directly on Constituents table in the current schema provided in core.ts
  // It seems 'isActive' might be on specific roles or maybe I missed it.
  // Looking at core.ts, Constituents table doesn't have isActive.
  // Organizations has isActive.
  // Assuming for now we just return all, or filter by email existence if requested.

  if (rule.filters?.hasEmail) {
    // Join with ContactInformations to ensure email exists
    const result = await dbClient.db
      .selectDistinct({ id: schema.Constituents.id })
      .from(schema.Constituents)
      .innerJoin(
        schema.ContactInformations,
        and(
          eq(schema.ContactInformations.constituentId, schema.Constituents.id),
