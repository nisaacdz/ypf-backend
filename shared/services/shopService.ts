 ...
 */
async function validateOrderItems(items: OrderItem[]): Promise<{
...
export async function createAuthenticatedOrder(
  ...
  try {
    const { sendOrderPlacementEmail } = await import("@/shared/utils/email");
...
export async function completeGuestOrder(
  ...
  const { validatedItems, totalAmount } = await validateOrderItems(payload.items);
